...
/**
 * Created by yuxinfeng on 17/3/30.
 */
...
public class BNRRNumberPickerView extends View {
    ...
    private int mSelectScaleDirection = MIDDLE_TO_BOTH_SIDE; // left，mid，right
    private boolean mDividerIgnorePadding = false; // left，mid，right
    ...
    public BNRRNumberPickerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initAttr(context, attrs);
        init(context);
    }

    public BNRRNumberPickerView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initAttr(context, attrs);
        init(context);
    }

    public void forbiddenScroll(boolean forbiddenScroll) {
        this.mForbiddenScroll = forbiddenScroll;
    }
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.NumberPickerView_npv_ShowCount) {
                ...
                mSelectScaleDirection = a.getInt(attr, MIDDLE_TO_BOTH_SIDE);
            } else if (attr == R.styleable.NumberPickerView_npv_DividerIgnorePadding) {
                mDividerIgnorePadding = a.getBoolean(attr, false);
            }
        }
        a.recycle();
    }
                ...
                switch (msg.what) {
                    case HANDLER_WHAT_REFRESH:
                        if (!mScroller.isFinished()) {
                            ...
                            mHandlerInNewThread.sendMessageDelayed(getMsg(HANDLER_WHAT_REFRESH, 0, 0, msg.obj),
                                    HANDLER_INTERVAL_REFRESH);
                        } else {
                            int duration = 0;
                            int willPickIndex;
                            ...
                            if (mCurrDrawFirstItemY != 0) { // need to adjust
                                ...
                                if (mCurrDrawFirstItemY < (-mItemHeight / 2)) {
                                    ...
                                    duration = (int) ((float) DEFAULT_INTERVAL_REVISE_DURATION
                                            * (mItemHeight + mCurrDrawFirstItemY) / mItemHeight);
                                    mScroller.startScroll(0, mCurrDrawGlobalY, 0, mItemHeight + mCurrDrawFirstItemY,
                                            duration * 3);
                                    willPickIndex = getWillPickIndexByGlobalY(
                                            mCurrDrawGlobalY + mItemHeight + mCurrDrawFirstItemY);
                                } else {
                                    // adjust to scroll downward
                                    duration = (int) ((float) DEFAULT_INTERVAL_REVISE_DURATION * (-mCurrDrawFirstItemY)
                                            / mItemHeight);
                            ...
                            Message changeMsg =
                                    getMsg(HANDLER_WHAT_LISTENER_VALUE_CHANGED, mPrevPickedIndex, willPickIndex,
                                            msg.obj);
                            if (LogUtil.LOGGABLE) {
                                log("HANDLER_WHAT_LISTENER_VALUE_CHANGED mPrevPickedIndex:" + mPrevPickedIndex);
                                log("HANDLER_WHAT_LISTENER_VALUE_CHANGED willPickIndex:" + willPickIndex);
                            }
    ...
    protected void onDetachedFromWindow() {
        ...
        if (currPickedIndex != mPrevPickedIndex && mRespondChangeOnDetach) {
            try {
                if (mOnValueChangeListener != null) {
                    mOnValueChangeListener.onValueChange(BNRRNumberPickerView.this, mPrevPickedIndex + mMinValue,
                            currPickedIndex + mMinValue);
                }
                if (mOnValueChangeListenerRaw != null) {
                    mOnValueChangeListenerRaw
                            .onValueChangeRelativeToRaw(BNRRNumberPickerView.this, mPrevPickedIndex, currPickedIndex,
                                    mDisplayedValues);
                }
            } catch (Exception e) {
                e.printStackTrace();
        ...
        log("onDetachedFromWindow mPrevPickedIndex:" + mPrevPickedIndex);
    }

    /**
     * 获取当前列的item total size
     *
     * @return
     */
    public int getOneRecycleSize() {
        if (LogUtil.LOGGABLE) {
            log("getOneRecycleSize mMaxShowIndex:" + mMaxShowIndex + ",mMinShowIndex:" + mMinShowIndex + ",final:"
                    + (mMaxShowIndex - mMinShowIndex + 1));
        }
    ...
    public void setDisplayedValuesAndPickedIndex(String[] newDisplayedValues, int pickedIndex, boolean needRefresh) {
        ...
        log("setDisplayedValuesAndPickedIndex mPrevPickedIndex:" + mPrevPickedIndex);
    ...
    public void setDisplayedValues(String[] newDisplayedValues) {
        ...
        log("setDisplayedValues mPrevPickedIndex:" + mPrevPickedIndex);
    ...
    public void smoothScrollToValue(int fromValue, int toValue, boolean needRespond) {
        ...
        scrollByIndexSmoothly(deltaIndex, needRespond, true);
    }

    public void smoothScrollToValueQuick(int fromValue, int toValue, boolean needRespond, boolean quick) {
        fromValue = refineValueByLimit(fromValue, mMinValue, mMaxValue,
                mWrapSelectorWheel && mWrapSelectorWheelCheck);
        toValue = refineValueByLimit(toValue, mMinValue, mMaxValue,
                mWrapSelectorWheel && mWrapSelectorWheelCheck);
        ...
        int deltaIndex = toValue - fromValue;
        scrollByIndexSmoothly(deltaIndex, needRespond, quick);
    }
    ...
    private void respondPickedValueChanged(int oldVal, int newVal, Object respondChange) {
        ...
        if (LogUtil.LOGGABLE) {
            ...
            log("respondPickedValueChanged oldVal:" + oldVal + ",newVal:" + newVal);
            log("respondPickedValueChanged respondChange:" + respondChange);
            if (respondChange != null) {
                log("respondPickedValueChanged respondChange is boolean:" + (respondChange instanceof Boolean));
            }
        }
            ...
            if (respondChange == null || !(respondChange instanceof Boolean) || (Boolean) respondChange) {
                if (mOnValueChangeListener != null) {
                    mOnValueChangeListener
                            .onValueChange(BNRRNumberPickerView.this, oldVal + mMinValue, newVal + mMinValue);
                }
                if (mOnValueChangeListenerRaw != null) {
                    mOnValueChangeListenerRaw
                            .onValueChangeRelativeToRaw(BNRRNumberPickerView.this, oldVal, newVal, mDisplayedValues);
                }
            }
        }
        log("respondPickedValueChanged mPrevPickedIndex:" + mPrevPickedIndex);
    ...
    public void resetPrevPickedIndex(int index) {
        this.mPrevPickedIndex = index;
    }

    private void scrollByIndexSmoothly(int deltaIndex) {
        scrollByIndexSmoothly(deltaIndex, true, true);
    }
     ...
     * @param needRespond need Respond to the ValueChange callback When Scrolling, default is false
     */
    private void scrollByIndexSmoothly(int deltaIndex, boolean needRespond, boolean needtime) {
        if (!(mWrapSelectorWheel && mWrapSelectorWheelCheck)) {
        ...
        if (mCurrDrawFirstItemY < (-mItemHeight / 2)) {
            ...
            duration = (int) ((float) DEFAULT_INTERVAL_REVISE_DURATION * (mItemHeight + mCurrDrawFirstItemY)
                    / mItemHeight);
            if (deltaIndex < 0) {
        ...
        if (needRespond) {
            ...
            mHandlerInNewThread
                    .sendMessageDelayed(getMsg(HANDLER_WHAT_REFRESH, 0, 0, new Boolean(needRespond)), duration / 4);
        }
        postInvalidate();
    }
        ...
        if (maxValue - mMinValue + 1 > mDisplayedValues.length) {
            throw new IllegalArgumentException(
                    "(maxValue - mMinValue + 1) should not be greater than mDisplayedValues.length now "
                            + " (maxValue - mMinValue + 1) is " + (maxValue - mMinValue + 1)
                            + " and mDisplayedValues.length is " + mDisplayedValues.length);
        }
        mMaxValue = maxValue;
    ...
    public void setPickedIndexRelativeToMin(int pickedIndexToMin) {
        if (0 <= pickedIndexToMin && pickedIndexToMin < getOneRecycleSize()) {
            ...
            log("setPickedIndexRelativeToMin mPrevPickedIndex:" + mPrevPickedIndex);
    ...
    public void setPickedIndexRelativeToRaw(int pickedIndexToRaw) {
        if (LogUtil.LOGGABLE) {
            log("setPickedIndexRelativeToRaw pickedIndexToRaw:" + pickedIndexToRaw);
            log("setPickedIndexRelativeToRaw mMaxShowIndex:" + mMaxShowIndex);
            log("setPickedIndexRelativeToRaw mMinShowIndex:" + mMinShowIndex);
            log("setPickedIndexRelativeToRaw (mMinShowIndex <= pickedIndexToRaw):"
                    + (mMinShowIndex <= pickedIndexToRaw));
            log("setPickedIndexRelativeToRaw (pickedIndexToRaw <= mMaxShowIndex):"
                    + (pickedIndexToRaw <= mMaxShowIndex));
        }
        if (mMinShowIndex > -1) {
            if (mMinShowIndex <= pickedIndexToRaw && pickedIndexToRaw <= mMaxShowIndex) {
                mPrevPickedIndex = pickedIndexToRaw;
                log("setPickedIndexRelativeToRaw mPrevPickedIndex:" + mPrevPickedIndex);
    ...
    public void setMinAndMaxShowIndex(int minShowIndex, int maxShowIndex, boolean needRefresh) {
        if (LogUtil.LOGGABLE) {
            log("setMinAndMaxShowIndex,params,minShowIndex:" + minShowIndex + ",maxShowIndex:" + maxShowIndex);
        }
        ...
        if (mDisplayedValues == null) {
            ...
            if (LogUtil.LOGGABLE) {
                log("setMinAndMaxShowIndex,mDisplayedValues.length - 1:" + (mDisplayedValues.length - 1));
            }
            if (minShowIndex < 0) {
                minShowIndex = 0;
            ...
            if (maxShowIndex < 0) {
                maxShowIndex = mDisplayedValues.length - 1;
            } else if (maxShowIndex > mDisplayedValues.length - 1) {
                maxShowIndex = mDisplayedValues.length - 1;
            }
        }
        if (LogUtil.LOGGABLE) {
            log("setMinAndMaxShowIndex,mMinShowIndex:" + mMinShowIndex + ",mMaxShowIndex:" + mMaxShowIndex);
        }
        mMinShowIndex = minShowIndex;
        mMaxShowIndex = maxShowIndex;
        if (needRefresh) {
            mPrevPickedIndex = 0 + mMinShowIndex;
            log("setMinAndMaxShowIndex mPrevPickedIndex:" + mPrevPickedIndex);
            correctPositionByDefaultValue(0, mWrapSelectorWheel && mWrapSelectorWheelCheck);
            postInvalidate();
        }
    }
    ...
    public void setFriction(float friction) {
        if (friction <= 0) {
            throw new IllegalArgumentException(
                    "you should set a a positive float friction, now friction is " + friction);
        }
    ...
    private int getWillPickIndexByGlobalY(int globalY) {
        ...
        int index =
                getIndexByRawIndex(willPickIndex, getOneRecycleSize(), mWrapSelectorWheel && mWrapSelectorWheelCheck);
        if (0 <= index && index < getOneRecycleSize()) {
            return index + mMinShowIndex;
        } else {
            if (LogUtil.LOGGABLE) {
                log("getWillPickIndexByGlobalY illegal index : " + index
                        + " getOneRecycleSize() : " + getOneRecycleSize()
                        + " gmWrapSelectorWheelCheck : " + mWrapSelectorWheelCheck
                        + " mMinShowIndex : " + mMinShowIndex
                        + " mWrapSelectorWheel : " + mWrapSelectorWheel);
            }
            return mMinShowIndex;
        }
    }
    ...
    private void updateDividerAttr() {
        ...
        if (getPaddingLeft() + mDividerMarginL >= mViewWidth - getPaddingRight() - mDividerMarginR) {
            ...
            mDividerMarginL = (int) (mDividerMarginL
                    - (float) surplusMargin * mDividerMarginL
                    / (mDividerMarginL + mDividerMarginR));
            mDividerMarginR = (int) (mDividerMarginR
                    - (float) surplusMargin * mDividerMarginR
                    / (mDividerMarginL + mDividerMarginR));
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (LogUtil.LOGGABLE) {
            log("onTouchEvent mForbiddenScroll:" + mForbiddenScroll);
        }
        if (mForbiddenScroll) {
            return true;
        }
        ...
        if (downYPos - curYPos < 0) {
            if (mInScrollingPickedNewValue < mIgnoreStartIndex) {
                smoothScrollToValue(mIgnoreStartIndex);
                return true;
            }
        }
        // end
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mFlagMayPress && (-mScaledTouchSlop < spanY && spanY < mScaledTouchSlop)) {
                    ...
                    log("ACTION_MOVE:" + mCurrDrawGlobalY);
                    calculateFirstItemParameterByGlobalY();
                    invalidate();
                }
                onScrollStateChange(OnScrollListener.SCROLL_STATE_TOUCH_SCROLL);
                ...
                if (mFlagMayPress) {
                    ...
                    if (Math.abs(velocityY) > mMiniVelocityFling) {
                        mScroller.fling(0, mCurrDrawGlobalY, 0, -velocityY,
                                Integer.MIN_VALUE, Integer.MAX_VALUE, limitY(Integer.MIN_VALUE),
                                limitY(Integer.MAX_VALUE));
                        invalidate();
    ...
    private void drawContent(Canvas canvas) {
        ...
        for (int i = 0; i < mShowCount + 1; i++) {
            ...
            if (0 <= index && index < getOneRecycleSize()) {
                // 不展示之前的时间
                if (index >= mIgnoreStartIndex) {
                    CharSequence str = mDisplayedValues[index + mMinShowIndex];
                    if (mTextEllipsize != null) {
                        str = TextUtils
                                .ellipsize(str, mPaintText, getWidth() - 2 * mItemPaddingHorizontal,
                                        getEllipsizeType());
                    }
                    setPaintAlign();
                    setTextCenterX(str.toString());
                    canvas.drawText(str.toString(), mTextCenterX, y + mItemHeight / 2 + textSizeCenterYOffset,
                            mPaintText);
                }
            } else if (!TextUtils.isEmpty(mEmptyItemHint)) {
                setPaintAlign();
                setTextCenterX(mEmptyItemHint);
                canvas.drawText(mEmptyItemHint, mTextCenterX, y + mItemHeight / 2 + textSizeCenterYOffset, mPaintText);
            }
        }
    }

    private void setPaintAlign() {
        if (mSelectScaleDirection == MIDDLE_TO_BOTH_SIDE) {
            mPaintText.setTextAlign(Align.CENTER);
        } else if (mSelectScaleDirection == RIGHT_TO_LEFT_SIDE) {
    ...
    private void setTextCenterX(String text) {
        log("setTextCenterX mViewCenterX:" + mViewCenterX);
        float defaultCenterX = mViewCenterX;
        if (mSelectScaleDirection == LEFT_TO_RIGHT_SIDE) {
            mTextCenterX = defaultCenterX - mMaxWidthOfDisplayedValues / 2;
        } else if (mSelectScaleDirection == RIGHT_TO_LEFT_SIDE) {
            mTextCenterX = defaultCenterX + defaultCenterX;
        } else {
            mTextCenterX = defaultCenterX;
        }
    }
    ...
    private void drawLine(Canvas canvas) {
        if (mShowDivider) {
            if (mDividerIgnorePadding) {
                canvas.drawLine(mDividerMarginL,
                        dividerY0, mViewWidth - mDividerMarginR, dividerY0, mPaintDivider);
                canvas.drawLine(mDividerMarginL,
                        dividerY1, mViewWidth - mDividerMarginR, dividerY1, mPaintDivider);
            } else {
    ...
    private void updateMaxHeightOfDisplayedValues() {
        ...
        mMaxHeightOfDisplayedValues =
                (int) (mPaintText.getFontMetrics().bottom - mPaintText.getFontMetrics().top + 0.5);
        mPaintText.setTextSize(savedTextSize);
    }
    ...
    private void log(String content) {
        if (getTag() != null) {
            LogUtil.e(TAG, "" + getTag() + "--" + content);
        }
    }

    public void setIgnoreStartIndex(int index) {
        mIgnoreStartIndex = index;
    }

    // compatible for NumberPicker
    public interface OnValueChangeListener {
        void onValueChange(BNRRNumberPickerView picker, int oldVal, int newVal);
    }

    public interface OnValueChangeListenerRelativeToRaw {
        void onValueChangeRelativeToRaw(BNRRNumberPickerView picker, int oldPickedIndex, int newPickedIndex,
                                        String[] displayedValues);
    }

    public interface OnValueChangeListenerInScrolling {
        void onValueChangeInScrolling(BNRRNumberPickerView picker, int oldVal, int newVal);
    }
    ...
    public interface OnScrollListener {
        ...
        void onScrollStateChange(BNRRNumberPickerView view, int scrollState);
    }
}
